...
async def _handle_text(chat_id: str, text: str, client: httpx.AsyncClient) -> None:
    ...
    if not business:
        await send_text(
            ...
            f"{get_settings().base_url} to sign up.",
        )
        return
...
async def _handle_link_phone(chat_id: str, phone_raw: str, client: httpx.AsyncClient) -> None:
    ...
    if not business:
        await send_text(
            ...
            f"Please check the number or visit {get_settings().base_url} to sign up.",
        )
        return
...
async def _handle_callback_query(callback: dict[str, Any], client: httpx.AsyncClient) -> None:
    ...
    if payload == "onboard_signup":
        ...
        await send_text(
            client, chat_id,
            f"🚀 Sign up at:\n👉 {settings.base_url}\n\n"
            f"Once signed up, come back and type /link to connect your account.",
        )
        return
